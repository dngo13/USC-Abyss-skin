  ...
  float ar = float(viewport.x) / float(viewport.y);
	...
	if (Bg == 1) {
		...
		for (int i=0;i<3;++i) {
			...
			if ((i < 2 && BgBaseTilt == 1) || i == 2) {
				float rotation = i < 2 // todo: tilt to maximum point
			...
			// ivec2 bgSize;
			// if (i < 2)
			// 	bgSize = textureSize(BackgroundTex, 1);
			// else 
			// 	bgSize = textureSize(LayerTex, 1);
			// float bgAr = bgSize.x / bgSize.y;
			// if (isAnim)
			// 	bgAr /= frameCount;
			...
			backgroundUV = mirrorTile(backgroundUV, .001, .999);
	...
	if (Center == 1) {
		...
		if (CenterTilt == 1)
			centerUV = rotatePoint(vec2(0.5,0.5), clamp(TunnelBaseRotation + (bgrot * TWO_PI),-360,360), centerUV);
		...
		if (CenterLayerEffect == 1) {
			...
			if (CenterLayerEffectGlow == 1) {
				a *= (GlowTimingProgress*.5+.4);
				sc *= (GlowTimingProgress*.03+.97);
			}
		...
		if (CenterPulse == 1) { // also has to account for clear version
			vec4 centerTextureef = texture(CenterTex, clamp(ScaleUV(centerUV,portrait(CenterScale,1.5)-fract(timing.z*1.5)*portrait(1,0.5),vec2(0.5)),0.0,1.0));
