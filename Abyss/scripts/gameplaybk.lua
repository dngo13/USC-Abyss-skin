...
function draw_song_info(deltaTime)
    ...
    gfx.ImageRect((desw / 3) - 55 , -5, tW/3.5, tH/3.5, topFillSm, 0.9, 0)
        ...
        gfx.DrawLabel(trackTitle, ((desw / 2)- (lbW/2)) , 5, 415)
    ...
    -- Draw the BPM
    gfx.FontSize(20)
    gfx.Text(string.format("BPM: %.1f", gameplay.bpm), textX, 45)
    -- Draw the hi-speed
    gfx.FontSize(20)
    gfx.Text(string.format("Hi-speed: %.1f", gameplay.hispeed), 110, 65)
    --tw, th = gfx.ImageSize(numberDot)
    gfx.BeginPath()
   
    -- Fill the progress bar
    gfx.FillColor(0, 150, 255)
    gfx.DrawRect(RECT_FILL, song_info.jacketWidth, song_info.jacketWidth - 10, (song_info.songInfoWidth - song_info.jacketWidth) * gameplay.progress , 9)
...
function draw_score(deltaTime)
    ...
    --gfx.Fill()
    --gfx.Stroke()
    gfx.ImageRect((desw-250), 5, 250, 128, scoreBack, 0.8, 0)
    gfx.Translate(-5, 5) -- upper right margin
    gfx.FillColor(255, 255, 255)
    gfx.TextAlign(gfx.TEXT_ALIGN_RIGHT + gfx.TEXT_ALIGN_TOP)
    gfx.FontSize(50)
    gfx.Text(string.format("%08d", score), desw, 20)
...
function draw_combo(deltaTime)
	...
	gfx.FontSize(24)
	gfx.FillColor(255, 255, 255)
	gfx.Text(string.format("%04d", maxChain), (desw+300) , 79.8)
