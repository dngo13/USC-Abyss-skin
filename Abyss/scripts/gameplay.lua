...
local topFillSm = gfx.CreateSkinImage("fill_top_small.png", 0)
...
local keysound = game.GetSkinSetting("button_sound")
local scoreBack = gfx.CreateSkinImage("scorebk.png", 0)
local songInfo = gfx.CreateSkinImage("song-info.png", 0)
local userName = game.GetSkinSetting("nick")
...
local late = false
local diffNames = {"NOVICE", "ADVANCE", "EXHAUST", "INFINITE"}
...
function render(deltaTime)
    ...
    --if not portrait then draw_smallBanner(deltaTime) end
...
--function draw_smallBanner(deltaTime)
  --  local bannerWidth, bannerHeight = gfx.ImageSize(topFillSm)
    --local actualHeight = desw * (bannerHeight / bannerWidth)

  --  gfx.FillColor(255, 255, 255)
    --gfx.DrawRect(topFill, 0, 0, desw, actualHeight)bannerWidth-500
   -- gfx.DrawRect(topFillSm,desw/2 , 5, bannerWidth/1.75, actualHeight/2)
   -- gfx.DrawRect(topFillSm, desw/2 - bannerWidth / 2 / 2 , 0, bannerWidth / 2, actualHeight/2)
  --  return actualHeight
--end
-- ---------------------------------------------------  ----------------------- --
-- draw_stat:                                                                 --
...
-- This function expects no graphics transform except the design scale.       --
local diffImg = gfx.CreateSkinImage("song_diff.png", 0)
...
function draw_song_info(deltaTime)
    ...
    -- Ensure the font has been loaded
     gfx.LoadSkinFont("Orbitron.ttf")
    --Draw Username
     local displayUser = gfx.CreateLabel(userName, 20, 0)

        -- USERNAME
        gfx.TextAlign(gfx.TEXT_ALIGN_LEFT)
        gfx.FillColor(0, 200, 255)
        gfx.DrawLabel(displayUser, 150, -5, 110)
        gfx.FontSize(24)

    gfx.LoadSkinFont("Quantify.ttf")
    --Draw the song info picture
    local imW, imH = gfx.ImageSize(songInfo)
    gfx.BeginPath()
    gfx.FillColor(200, 200, 255)
    gfx.ImageRect(-5, -5, imW/4, imH/4, songInfo, 1, 0)
    
    -- Draw the jacket
    gfx.FillColor(255, 255, 255)
    gfx.DrawRect(jacket, 8, 5, song_info.jacketWidth, song_info.jacketWidth)
    gfx.Save()
    gfx.BeginPath()
    ...
    --gfx.FillColor(255, 255, 255)
    gfx.FontSize(22)
    local songDiffW, songDiffH = gfx.ImageSize(diffImg)
    gfx.FillColor(200, 200, 255)
    gfx.ImageRect(-24, 100, ((songDiffW/2) - 20), songDiffH/2, diffImg, 1, 0)
    gfx.FillColor(255, 255, 255)
    draw_stat(13, 105, 90, 16, diffNames[gameplay.difficulty + 1], gameplay.level, "%02d")
    ...
    local yshift = 5
    gfx.Save()
    local tW, tH = gfx.ImageSize(topFillSm)
    local trackTitle = gfx.CreateLabel(gameplay.title .. " - " .. gameplay.artist, 18, 0)
    local lbW, lbH = gfx.LabelSize(trackTitle)
    gfx.FillColor(255, 255, 255)
    gfx.BeginPath()
    --gfx.ImageRect((desw / 5) + 150, -5, tW/4, tH/4, topFillSm, 1, 0)
    gfx.ImageRect((desw / 3) - 55 , -5, tW/3.5, tH/3.5, topFillSm, 0.9, 0)
    --gfx.DrawLabel((desw/2) - 20), -29, 415)
    gfx.DrawLabel(trackTitle, ((desw / 2) - lbW/2) , 5, 600)
    ...
    -- Draw the BPM
    gfx.FontSize(20)
    gfx.Text(string.format(" %.1f", gameplay.bpm), textX+60, 78)
    -- Draw the hi-speed
    gfx.FontSize(20)
    gfx.Text(string.format("%.1f", gameplay.hispeed), 215, 100)
    --tw, th = gfx.ImageSize(numberDot)
    gfx.BeginPath()
   
    -- Fill the progress bar
    gfx.FillColor(0, 150, 255)
    gfx.DrawRect(RECT_FILL, 119, 45, 147 * gameplay.progress , 7)
...
function draw_score(deltaTime)
    ...
    gfx.BeginPath()
       gfx.FillColor(200, 200, 255)
    gfx.ImageRect((desw-265), -5, 260, 150, scoreBack, 1, 0)
    gfx.Translate(-5, 5) -- upper right margin
    gfx.FillColor(255, 255, 255)
    gfx.TextAlign(gfx.TEXT_ALIGN_RIGHT + gfx.TEXT_ALIGN_TOP)
    gfx.FontSize(50)
    gfx.Text(string.format("%08d", score), desw-10, 20)
...
function draw_combo(deltaTime)
	...
	gfx.FontSize(22)
	gfx.FillColor(255, 255, 255)
	gfx.Text(string.format("%04d", maxChain), (desw - 150), 85)
...
-- key_sound:  
function key_sound(deltaTime)
    if not keysound == true or introTimer ~=0 then return end
    if (not bta_last) and game.GetButton(game.BUTTON_BTA) or
	(not btb_last) and game.GetButton(game.BUTTON_BTB) or
	(not btc_last) and game.GetButton(game.BUTTON_BTC) or
	(not btd_last) and game.GetButton(game.BUTTON_BTD) or
	(not fxl_last) and game.GetButton(game.BUTTON_FXL) or
	(not fxr_last) and game.GetButton(game.BUTTON_FXR) then
	game.PlaySample("tick")
	end
		
	bta_last = game.GetButton(game.BUTTON_BTA)
	btb_last = game.GetButton(game.BUTTON_BTB)
	btc_last = game.GetButton(game.BUTTON_BTC)
	btd_last = game.GetButton(game.BUTTON_BTD)
	fxl_last = game.GetButton(game.BUTTON_FXL)
	fxr_last = game.GetButton(game.BUTTON_FXR)
end
