...
draw_song = function(song, x, y, w, h, selected, difficulty)
    local diff = song.difficulties[selectedDiff]
    check_or_create_cache(song)
    gfx.BeginPath()
    gfx.RoundedRectVarying(x,y, w, h,0,0,0,80)
    gfx.FillColor(25,25,25, 200)
    gfx.StrokeColor(0,203,255)
    gfx.StrokeWidth(1)
    if selected then
        gfx.StrokeColor(208,288,245)
        gfx.StrokeWidth(2)
    end
      -- Draw the background
  --gfx.FillColor(255, 255, 255)
   ...
   -- gfx.LoadImageJob(diff.jacketPath, jacketFallback, 200,200)
    --local jacket = diff.jacketPath
    --jacket:draw({ x = 15, y = y+300, w = 50, h = 50 })
...
draw_selected = function(song, x, y, w, h)
  ...
  --iconimg:draw({ x = 638, y = 118, w = 70, h = 70, anchor_h = Image.ANCHOR_LEFT, anchor_v = Image.ANCHOR_TOP })
  ...
  gfx.FillColor(255, 255, 255)
  gfx.ImageRect(0, 0, 70, 70, iconimg, 1, 0)
    ...
    if aspectRatio == "portrait" then
      ...
      --gfx.FontSize(20)
      --gfx.DrawLabel(string.format("Path: %s", path), xpos+xPadding+imageSize+3, y+yMargin+yPadding + 125, width-imageSize-20)
      gfx.ImageRect(100, 100, 80, 80, iconimg, 1, 0)
      gfx.FillColor(255, 255, 255)
    else
      gfx.LoadSkinFont("Rondalo.ttf");
      gfx.FontSize(40)
      ...
      gfx.FillColor(255,255,255)
      gfx.FontSize(20)
      gfx.DrawLabel(songCache[song.id]["bpm"], xpos+10, (height/10)*6 + 75)
      gfx.FastText(string.format("Effector: %s", diff.effector),xpos+10, (height/10)*6 + 95)
      --gfx.FontSize(20)
      gfx.FillColor(255, 255, 255)
      gfx.BeginPath()
      gfx.ImageRect(550, (height/10)*6 +40, 100, 100, iconimg, 1, 0)
      gfx.FastText(string.format("Folder: %s", folderName), xpos+10, (height/10)*6 + 115)
      --gfx.FastText(string.format("Path: %s", path), xpos+10, (height/10)*6 + 135, width-20)
    end
    if aspectRatio == "portrait" then
...
recalc_page = function(idax)
  local pageStart = math.max(1 + idax * rowSize, 1)
  local pageEnd = math.min((idax + 3) * rowSize, #songwheel.songs)
...
render = function(deltaTime)
    ...
    if songwheel.songs[1] ~= nil then
      ...
      if aspectRatio == "portrait" then
        ...
        draw_sort_legend(0, resy * 2/9, resx, resy/10)
      else
        scrollX = resx - ((resx / 2) - (resx/2.05))
        scrollY = resy / 8
        scrollH = resy*6/8
        song = draw_songwheel(resx/2, scrollY, resx/2.05, resy*6/8)
        draw_selected(song, 0, 0, resx/2, resy/4)
        draw_sort_legend(resx/2, 0, resx/2, resy/8)
      end
